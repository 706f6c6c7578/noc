    ...
    if dataFile != "" {
        ...
        for _, addr := range addresses {
            ...
            if !strings.HasPrefix(serverAddress, "http://") && !strings.HasPrefix(serverAddress, "https://") {
                serverAddress = "https://" + serverAddress // Standardmäßig HTTPS
            }
        ...
        args := flag.Args()
        if len(args) != 2 {
            fmt.Println("Usage: noc [-u username] [-d datafile] [-h hide server response] \n          -f <filename> <server_address:port> <password>")
            os.Exit(1)
        }
        serverAddress, password := args[0], args[1]
        if !strings.HasPrefix(serverAddress, "http://") && !strings.HasPrefix(serverAddress, "https://") {
            serverAddress = "https://" + serverAddress // HTTPS by default
        }
        serverURL := serverAddress + "/upload"
        err = uploadFile(serverURL, password, username, filename, hideResponse)
        if err != nil {
...
func readDataFile(filename string) ([][]string, error) {
    ...
    for scanner.Scan() {
        ...
        // Split line and filter empty parts
...
func uploadFile(serverURL, password, username, filename string, hideResponse bool) error {
    ...
    // SOCKS5 dialer for the Nym Mix Network (port 1080)
    dialer, err := proxy.SOCKS5("tcp", "localhost:1080", nil, proxy.Direct)
    if err != nil {
        return fmt.Errorf("can't connect to the Nym proxy: %v", err)
    }
